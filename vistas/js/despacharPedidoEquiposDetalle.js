...
function genera_tabla_pedido() {

	var idObra = $('#idObra').val();

	$('.tablaPedidosCategoriasGuiaDespacho').DataTable({
		"ajax": {
			"url": "ajax/datatable-pedido-equipo-despachar.ajax.php",
			"method": 'POST',
			"datatype": 'json',
			"destroy": "true",
			"data": {
...
$(".tablaPedidosCategoriasGuiaDespacho").on("click", ".btnCategoriaSel", function() {
	...
	$('#idRegistroPedidoObra').val(idPedido);
	$('#idCategoria').val(idCategoria);
	$('.tablaEquiposGuia').DataTable().destroy();

	recargaTabla(idCategoria);
...
$('#btnAgregarEquipo').click(function() {
	...
	if ($('#idRegistroPedidoObra').val() == '') {
		alertify.error("Seleccione un equipo del pedido");
		return false;
	}
	...
	idPedidoObra = $('#idRegistroPedidoObra').val();
	...
	$.ajax({
		...
		success: function(res) {
			...
			$('.tablaEquiposGuia').DataTable().ajax.reload();
			 ...
			 $('.tablaPedidosCategoriasGuiaDespacho').DataTable().destroy();
			genera_tabla_pedido();
			$('.tablaPedidosCategoriasGuiaDespacho').DataTable().ajax.reload();
...
function recargaTabla(id) {
	...
	$('.tablaEquiposGuia').DataTable({
		"ajax": {
			"url": "ajax/datatable-detalle-equipos-guia-con-pedido.ajax.php",
			"method": 'POST',
			"datatype": 'json',
			"destroy": "true",
			"data": {
