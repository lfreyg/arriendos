...
class ModeloGuiaDespachoTaller{
	...
	static public function mdlMostrarGuiaDespacho($idSucursal){
			...
			$stmt = Conexion::conectar()->prepare("SELECT gd.id as id, eo.razon_social as empresa, gd.numero_guia as guia, gd.fecha_guia as fecha, t.nombre as taller, gd.estado_guia as idestado, e.descripcion as estado, tg.id as idtransporte, tg.nombre as chofer FROM guia_despacho gd JOIN empresas_operativas eo ON gd.id_empresa = eo.id JOIN talleres t ON gd.id_constructoras = t.id JOIN estados e ON gd.estado_guia = e.id LEFT JOIN transporte_guia tg ON gd.id_transporte_guia = tg.id  where gd.id_sucursal = $idSucursal and gd.tipo_guia = 'TA' order by gd.id desc");
	...
	static public function mdlMostrarGuiaDespachoTallerDetalle($id){

		
			
			$stmt = Conexion::conectar()->prepare("SELECT gd.estado_guia as estadoGuia, eo.id as idEmpresa, eo.razon_social as empresa, gd.numero_guia as guia, gd.fecha_guia as fecha, t.rut as rutConstructora, t.nombre as constructora, gd.rut_empresa_transporte as rutTransporte, gd.patente_transportista as patente, gd.rut_transportista as rutChofer, gd.nombre_transportista as chofer, gd.id_constructoras as idConstructora, gd.id_obras as idObra FROM guia_despacho gd JOIN empresas_operativas eo ON gd.id_empresa = eo.id JOIN talleres t ON gd.id_constructoras = t.id where gd.id = $id and gd.tipo_guia = 'TA'");
	...
	static public function mdlIngresarGuiaDespacho($tabla, $datos){
		...
		$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(id_empresa, fecha_guia, id_constructoras, id_sucursal, id_transporte_guia, rut_empresa_transporte, rut_transportista, nombre_transportista, patente_transportista, creado_por, tipo_guia) VALUES (:id_empresa,:fecha_guia,:id_constructora, :id_sucursal, :id_transporte_guia, :rut_empresa_transporte, :rut_transportista, :nombre_transportista, :patente_transportista, :creado_por, :tipoGuia)");
	...
	static public function mdlEditarGuiaDespachoTaller($tabla, $datos){

		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET fecha_guia = :fecha_guia, id_constructoras = :id_constructora, id_transporte_guia =:id_transporte_guia, rut_empresa_transporte =:rut_empresa_transporte, rut_transportista =:rut_transportista, nombre_transportista =:nombre_transportista, patente_transportista =:patente_transportista where id = :id");
	...
	static public function mdlEliminarGuiaDespachoTaller($id){
		...
		if($stmt -> execute()){
			...
			$stmt3 = Conexion::conectar()->prepare("UPDATE log_cambia_estados SET id_guia_despacho_envia = null WHERE id_guia_despacho_envia = $id");          		
		        $stmt3 -> execute();		
	...
	static public function mdlAnularGuiaDespachoTaller($id, $idUsuario){
